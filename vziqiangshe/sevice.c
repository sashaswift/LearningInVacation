        ...
        while(cardlis!=NULL){
                if(strstr(cardlis->t.num,num)!=NULL&&(cardlis->t.del==0)){
                 ...
                 cardlis->t.del=1;
                        ...
                        printf("%-20s\t%d\t\t%.2lf\t\t%.2lf\t\t%d\t\t",cardlis->t.num,cardlis->t.status,cardlis->t.rest,cardlis->t.already,cardlis->t.times);
    ...
    if(getBilling()){
        ...
        while(pbill!=NULL){
            if(strcmp(pbill->data.aCardName,pNum)==0&&((pbill->data.tEnd>=startdate&&pbill->data.tEnd<=enddate)||(pbill->data.tStart>=startdate&&pbill->data.tStart<=enddate))&&pbill->data.nDel==0){
                ...
                printf("%-20s\t%.2lf\t\t",pbill->data.aCardName,pbill->data.fAmount);
